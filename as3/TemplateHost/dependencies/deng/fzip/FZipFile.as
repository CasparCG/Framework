...
package deng.fzip
{
	...
	public class FZipFile
	{
		...
		public function get content():ByteArray {
			if(isCompressed) {
				System.println("uncompressing: " + this.filename);
		...
		public function getContentAsString(recompress:Boolean = true, charset:String = "utf-8"):String {
			if(isCompressed) {
				System.println("uncompressing2:" + this.filename);
				uncompress();
			}
			_content.position = 0;
			var str:String;
...
protected function uncompress():void {
	if(isCompressed && _content.length > 0) {
		...
		if(HAS_INFLATE) {
			...
			//d_stream.next_in_index = 1;
			//d_stream.istate.method = this._compressionMethod;
			//d_stream.istate.mode = Inflate.BLOCKS
			...
			while(d_stream.total_in <= _content.length && i<=_content.length*4) {
				...
				//System.println(this.filename + ":" + i);
				//if (false && i==0) {
				//	for (var tmp:int=0; tmp<50; tmp++) {
				//		System.println("\t" + tmp + ":" + _content[tmp]);
				//	}
				//} 
			
				d_stream.avail_in=d_stream.avail_out=10; // force small buffers 
				...
				//System.println("" +d_stream.total_out);
